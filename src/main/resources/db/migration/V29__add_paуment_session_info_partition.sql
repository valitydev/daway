--payment_session_info partition
DO
$$
    DECLARE
table_name              TEXT     := 'payment_session_info';
        is_partitioned
BOOLEAN;
        partition_name
TEXT;
...
ELSE
            ...
            --- Продолжим использовать предыдущую последовательность (payment_session_info_id_seq), поэтому используем BIGINT вместо BIGSERIAL
            EXECUTE format('CREATE TABLE IF NOT EXISTS dw.%I_new
                            (
                                id                                BIGINT                   NOT NULL DEFAULT nextval(''dw.payment_session_info_id_seq''::regclass),
