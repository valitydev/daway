--invoice_status_info partition
DO
$$
    DECLARE
table_name              TEXT     := 'invoice_status_info';
        is_partitioned
BOOLEAN;
        partition_name
TEXT;
...
ELSE
            ...
            --- Продолжим использовать предыдущую последовательность (invoice_status_info_id_seq), поэтому используем BIGINT вместо BIGSERIAL
            EXECUTE format('CREATE TABLE IF NOT EXISTS dw.%I_new
                            (
                                id               BIGINT                                         NOT NULL DEFAULT nextval(''dw.invoice_status_info_id_seq''::regclass),
